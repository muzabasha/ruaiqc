...
5. The 10 output neurons vote on which digit it is (0 through 9).`,
  ...
  learningResource: {
    ...
    description: 'Convolutional Neural Networks, MNIST dataset, backpropagation, loss functions, and optimization.',
    lessonNumber: 5,
    pages: 12,
    author: 'Dr. Syed Muzamil Basha',
  },
  pythonHandsOn: {
    title: 'PyTorch CNN Handwritten Digit Classifier (MNIST)',
    description: 'Build, train, and evaluate a Convolutional Neural Network (CNN) with Conv2d, MaxPool2d, and Linear layers on the MNIST dataset using PyTorch.',
    packages: ['torch', 'torchvision', 'matplotlib'],
    installCommand: 'pip install torch torchvision matplotlib',
    imports: [
      { code: 'import torch', explanation: 'Core PyTorch tensor and autograd library' },
      { code: 'import torch.nn as nn', explanation: 'Neural network layers (Conv2d, MaxPool2d, Linear)' },
      { code: 'import torchvision.transforms as transforms', explanation: 'Image transformations and normalization' },
      { code: 'import torchvision.datasets as datasets', explanation: 'MNIST benchmark dataset loader' },
      { code: 'from torch.utils.data import DataLoader', explanation: 'Mini-batch generator' },
      { code: 'import matplotlib.pyplot as plt', explanation: 'Plotting utility' },
    ],
    code: [
      { code: '# Transform: Convert image to Tensor and normalize pixels to [-1, 1]', explanation: 'Normalization pipeline' },
      { code: 'transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))])', explanation: 'Normalize with mean 0.5 and std 0.5' },
      { code: '', explanation: '' },
      { code: '# Download and prepare MNIST training and test datasets', explanation: 'Data loading' },
      { code: "train_data = datasets.MNIST(root='./data', train=True, download=True, transform=transform)", explanation: 'Load 60,000 training images' },
      { code: "test_data = datasets.MNIST(root='./data', train=False, download=True, transform=transform)", explanation: 'Load 10,000 test images' },
      { code: '', explanation: '' },
      { code: '# Create mini-batch DataLoaders (batch size = 64)', explanation: 'DataLoaders' },
      { code: 'train_loader = DataLoader(train_data, batch_size=64, shuffle=True)', explanation: 'Shuffle training batches' },
      { code: 'test_loader = DataLoader(test_data, batch_size=64, shuffle=False)', explanation: 'Test batches' },
      { code: '', explanation: '' },
      { code: '# Define Convolutional Neural Network (CNN) Architecture', explanation: 'CNN definition' },
      { code: 'class CNN(nn.Module):', explanation: 'Subclass nn.Module' },
      { code: '    def __init__(self):', explanation: 'Constructor' },
      { code: '        super(CNN, self).__init__()', explanation: 'Initialize parent' },
      { code: '        self.conv1 = nn.Conv2d(1, 32, kernel_size=3, stride=1, padding=1)', explanation: '32 feature filters (3x3)' },
      { code: '        self.pool = nn.MaxPool2d(2, 2)', explanation: '2x2 max-pooling (downsamples 28x28 to 14x14)' },
      { code: '        self.fc1 = nn.Linear(32 * 14 * 14, 10)', explanation: 'Dense output layer mapping to 10 classes' },
      { code: '', explanation: '' },
      { code: '    def forward(self, x):', explanation: 'Forward computation' },
      { code: '        x = self.pool(torch.relu(self.conv1(x)))', explanation: 'Conv -> ReLU -> MaxPool' },
      { code: '        x = x.view(-1, 32 * 14 * 14)', explanation: 'Flatten feature maps' },
      { code: '        x = self.fc1(x)', explanation: 'Linear output logits' },
      { code: '        return x', explanation: 'Return predictions' },
      { code: '', explanation: '' },
      { code: 'model = CNN()', explanation: 'Instantiate CNN model' },
      { code: 'criterion = nn.CrossEntropyLoss()', explanation: 'Cross-entropy loss function' },
      { code: 'optimizer = torch.optim.Adam(model.parameters(), lr=0.001)', explanation: 'Adam optimizer with lr=0.001' },
      { code: '', explanation: '' },
      { code: '# Train for 5 epochs over the MNIST dataset', explanation: 'Training loop' },
      { code: 'for epoch in range(5):', explanation: 'Loop through epochs' },
      { code: '    for images, labels in train_loader:', explanation: 'Iterate mini-batches' },
      { code: '        optimizer.zero_grad()', explanation: 'Clear previous gradients' },
      { code: '        outputs = model(images)', explanation: 'Forward pass' },
      { code: '        loss = criterion(outputs, labels)', explanation: 'Compute batch loss' },
      { code: '        loss.backward()', explanation: 'Backpropagate error' },
      { code: '        optimizer.step()', explanation: 'Update weights' },
      { code: '    print(f"Epoch {epoch+1}, Loss: {loss.item():.4f}")', explanation: 'Log epoch progress' },
      { code: '', explanation: '' },
      { code: '# Evaluate model accuracy on unseen test set (10,000 images)', explanation: 'Testing loop' },
      { code: 'correct = 0', explanation: 'Counter for correct predictions' },
      { code: 'total = 0', explanation: 'Total test samples' },
      { code: 'with torch.no_grad():', explanation: 'Disable gradient tracking for inference' },
      { code: '    for images, labels in test_loader:', explanation: 'Loop over test batches' },
      { code: '        outputs = model(images)', explanation: 'Predict on test images' },
      { code: '        _, predicted = torch.max(outputs.data, 1)', explanation: 'Select highest logit index' },
      { code: '        total += labels.size(0)', explanation: 'Tally batch size' },
      { code: '        correct += (predicted == labels).sum().item()', explanation: 'Count correct matches' },
      { code: '', explanation: '' },
      { code: 'print(f"Test Accuracy: {100 * correct / total:.2f}%")', explanation: 'Display final test accuracy' },
    ],
    executionFlow: [
      { number: 1, title: 'Data Loading & Normalization', description: 'Downloads 70,000 MNIST images and scales grayscale pixels to [-1, 1] range.' },
      { number: 2, title: 'Spatial Feature Extraction', description: 'Conv2d layer extracts 32 feature maps, followed by ReLU and 2x2 max-pooling to downsample spatial dimensions.' },
      { number: 3, title: 'Mini-Batch Optimization', description: 'Adam optimizer minimizes CrossEntropyLoss across mini-batches of 64 images.' },
      { number: 4, title: 'Unseen Test Generalization', description: 'Evaluates on 10,000 held-out test images, achieving >98% test accuracy.' },
    ],
    input: '60,000 training and 10,000 test handwritten digit images (28x28 grayscale pixels)',
    output: 'Epoch 1, Loss: 0.0842\nEpoch 2, Loss: 0.0511\nEpoch 3, Loss: 0.0324\nEpoch 4, Loss: 0.0195\nEpoch 5, Loss: 0.0128\nTest Accuracy: 98.65%',
    interpretation: 'The convolutional neural network effectively captures 2D spatial stroke geometries, correctly classifying unseen handwritten digits with over 98% accuracy.',
    colabInstructions: ['Run in Google Colab with GPU acceleration for rapid training.'],
  },
  mcqs: [
    {
      id: 'q1',
