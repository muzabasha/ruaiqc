...
In the 2D subspace spanned by $|\\omega\\rangle$ and $|s'\\rangle = \\frac{1}{\\sqrt{N-1}}\\sum_{x \\ne \\omega}|x\\rangle$, $G$ executes a planar rotation by angle $\\theta = 2\\arcsin(1/\\sqrt{N}) \\approx 2/\\sqrt{N}$.`,
  ...
  pythonHandsOn: {
    ...
    description: 'Implement Grover’s quantum search algorithm to find target state |11> using Oracle (CZ) and Diffusion Operator functions composed onto a 2-qubit circuit with Aer simulation and histogram plotting.',
    ...
    imports: [
      ...
      { code: 'from qiskit.visualization import plot_histogram', explanation: 'Visualize measurement distribution' },
      { code: 'from qiskit_aer import Aer', explanation: 'Import Aer simulator backend' },
      { code: 'import matplotlib.pyplot as plt', explanation: 'Matplotlib plotting backend' },
    ],
    code: [
      { code: '# Define the Oracle (marks the |11⟩ state)', explanation: 'Oracle function' },
      { code: 'def grover_oracle():', explanation: 'Function definition' },
      { code: '    oracle = QuantumCircuit(2)', explanation: '2-qubit circuit for oracle' },
      { code: '    oracle.cz(0, 1)  # Controlled-Z gate to mark |11⟩', explanation: 'Controlled-Z adds negative phase to |11>' },
      { code: '    return oracle', explanation: 'Return oracle circuit' },
      { code: '', explanation: '' },
      { code: '# Define the Diffusion Operator (amplifies the correct state)', explanation: 'Diffusion function' },
      { code: 'def diffusion_operator():', explanation: 'Function definition' },
      { code: '    diffusion = QuantumCircuit(2)', explanation: '2-qubit circuit for diffusion' },
      { code: '    diffusion.h([0, 1])  # Apply Hadamard gates', explanation: 'H on all wires' },
      { code: '    diffusion.x([0, 1])  # Apply X gates', explanation: 'X on all wires' },
      { code: '    diffusion.cz(0, 1)   # Apply Controlled-Z gate', explanation: 'Controlled phase inversion' },
      { code: '    diffusion.x([0, 1])  # Apply X gates again', explanation: 'Uncompute X' },
      { code: '    diffusion.h([0, 1])  # Apply Hadamard gates again', explanation: 'Uncompute H' },
      { code: '    return diffusion', explanation: 'Return diffusion circuit' },
      { code: '', explanation: '' },
      { code: '# Create a 2-qubit quantum circuit', explanation: 'Main algorithm circuit' },
      { code: 'qc = QuantumCircuit(2, 2)', explanation: 'Allocate 2 qubits and 2 classical bits' },
      { code: '', explanation: '' },
      { code: '# Step 1: Initialize with Hadamard gates', explanation: 'Equal superposition' },
      { code: 'qc.h([0, 1])', explanation: 'Put register into uniform superposition' },
      { code: '', explanation: '' },
      { code: '# Step 2: Apply Grover\'s Oracle using .compose()', explanation: 'Mark target state' },
      { code: 'qc = qc.compose(grover_oracle())', explanation: 'Append oracle sub-circuit' },
      { code: '', explanation: '' },
      { code: '# Step 3: Apply the Diffusion Operator using .compose()', explanation: 'Amplify marked amplitude' },
      { code: 'qc = qc.compose(diffusion_operator())', explanation: 'Append diffusion sub-circuit' },
      { code: '', explanation: '' },
      { code: '# Step 4: Measure the qubits', explanation: 'Readout' },
      { code: 'qc.measure([0, 1], [0, 1])', explanation: 'Measure qubits into classical bits' },
      { code: '', explanation: '' },
      { code: '# Execute on Aer simulator', explanation: 'Run simulation' },
      { code: 'simulator = Aer.get_backend(\'aer_simulator\')', explanation: 'Load aer_simulator backend' },
      { code: 'job = simulator.run(qc, shots=1000)', explanation: 'Execute 1,000 shots' },
      { code: 'result = job.result()', explanation: 'Get simulation results' },
      { code: 'counts = result.get_counts(qc)', explanation: 'Retrieve measurement counts' },
      { code: 'print("Grover Search Measurement Results (Lesson 9):", counts)', explanation: 'Display counts (100% |11>)' },
      { code: 'plot_histogram(counts)', explanation: 'Render histogram of measurement counts' },
      { code: 'plt.show()', explanation: 'Display figure' },
    ],
    executionFlow: [
      { number: 1, title: 'Superposition Initialization', description: 'Apply Hadamard gates to initialize both qubits into uniform equal superposition.' },
      { number: 2, title: 'Oracle Invocation', description: 'The CZ gate adds a -1 phase to target state |11>, marking it without altering classical probabilities.' },
      { number: 3, title: 'Diffusion Transformation', description: 'The H-X-CZ-X-H sequence inverts amplitudes about their mean, amplifying |11> to 100% amplitude.' },
      { number: 4, title: 'Measurement Verification', description: '1,000 shots on AerSimulator reveal 100% probability for the marked target |11> in a single query.' },
    ],
    input: '2-qubit circuit initialized to |00>, CZ oracle targeting |11>, and Grover diffusion operator.',
    output: "Grover Search Measurement Results (Lesson 9): {'11': 1000}",
    interpretation: 'Demonstrates quadratic speedup: in an unsorted 4-item search space, Grover algorithm finds target |11> in exactly one quantum iteration with 100% deterministic success.',
    colabInstructions: [
      'Click "Copy for Google Colab" to copy the complete runnable script',
      'Open Google Colab at colab.research.google.com',
      'Paste into a code cell and press Shift + Enter to run',
  ...
  learningResource: {
    ...
    description: 'Oracle, diffusion operator, interference, and quadratic search speedup — Presentation by Dr. Syed Muzamil Basha.',
    lessonNumber: 9,
    pages: 10,
