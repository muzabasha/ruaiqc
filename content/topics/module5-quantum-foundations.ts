...
- When the crest of one wave meets the trough of another wave, they cancel each other out and the water becomes completely flat (Destructive Interference)!`,
  ...
  pythonHandsOn: {
    title: "Extracting Complex Amplitudes and Testing the Born Rule in Qiskit",
    description: "Construct a quantum state with complex phase amplitudes and verify that measurement probabilities equal |alpha|^2 and |beta|^2.",
    packages: ["qiskit", "numpy"],
    installCommand: "pip install qiskit numpy",
    imports: [
    ...
    code: [
      { code: "# Create a state with complex phase: apply H then S gate (pi/2 phase)", explanation: "Complex state prep" },
      { code: "qc = QuantumCircuit(1)", explanation: "Single qubit" },
      { code: "qc.h(0)  # Creates (|0> + |1>)/sqrt(2)", explanation: "Hadamard gate" },
      { code: "qc.s(0)  # Adds i phase to |1>: (|0> + i|1>)/sqrt(2)", explanation: "S phase gate" },
      { code: "sv = Statevector.from_instruction(qc)", explanation: "Extract statevector" },
      { code: "alpha, beta = sv.data[0], sv.data[1]", explanation: "Extract complex amplitudes" },
      { code: "print(f'Complex Amplitudes:')", explanation: "Label" },
      { code: "print(f'  alpha = {alpha.real:.4f} + {alpha.imag:.4f}j')", explanation: "Alpha amplitude" },
      { code: "print(f'  beta  = {beta.real:.4f} + {beta.imag:.4f}j')", explanation: "Beta amplitude" },
      { code: "", explanation: "" },
      { code: "# Born Rule: Probability = |amplitude|^2", explanation: "Born rule evaluation" },
      { code: "prob_0 = np.abs(alpha)**2", explanation: "Squared magnitude of alpha" },
      { code: "prob_1 = np.abs(beta)**2", explanation: "Squared magnitude of beta" },
      { code: "print(f'\\nBorn Rule Probabilities:')", explanation: "Probabilities header" },
      { code: "print(f'  P(0) = |alpha|^2 = {prob_0:.4f} ({prob_0*100:.1f}%)')", explanation: "P(0)" },
      { code: "print(f'  P(1) = |beta|^2  = {prob_1:.4f} ({prob_1*100:.1f}%)')", explanation: "P(1)" },
      { code: "print(f'  Total Probability: {prob_0 + prob_1:.4f} (Exact Normalization)')", explanation: "Sum verification" }
    ],
    executionFlow: [
      { number: 1, title: "Create Complex State", description: "Use H and S gates to generate state (|0> + i|1>)/sqrt(2)." },
      { number: 2, title: "Extract Amplitudes", description: "Extract complex numbers alpha and beta from Qiskit Statevector." },
      { number: 3, title: "Compute Born Rule", description: "Calculate absolute value squared to prove probabilities sum to exactly 1.0." }
    ],
    input: "QuantumCircuit(1) with H and S gates.",
    output: "alpha = 0.7071 + 0.0000j\nbeta  = 0.0000 + 0.7071j\nP(0) = 0.5000, P(1) = 0.5000\nTotal Probability: 1.0000",
    interpretation: "Probability amplitudes in quantum mechanics are complex numbers. The Born Rule connects the microscopic complex amplitude to macroscopic physical observation by taking the squared magnitude.",
    colabInstructions: [
      "Click 'Copy for Google Colab' to copy the complete runnable script",
      "Open Google Colab at colab.research.google.com",
      "Paste into a code cell and press Shift + Enter to run",
...
Measuring qubit A yields 0 or 1 with 50% probability, but instantly projects qubit B into the identical state with correlation coefficient 1.0, violating the Bell-CHSH inequality ($S \\le 2$ classical vs $S = 2\\sqrt{2} \\approx 2.828$ quantum).`,
  ...
  pythonHandsOn: {
    title: "Generating the 4 Canonical Bell States in Qiskit",
    description: "Construct and analyze all four maximally entangled 2-qubit Bell states (|Phi+>, |Phi->, |Psi+>, |Psi->) using Qiskit.",
    packages: ["qiskit", "qiskit-aer"],
    installCommand: "pip install qiskit qiskit-aer",
    imports: [
      { code: "from qiskit import QuantumCircuit", explanation: "Circuit constructor" },
    ...
    code: [
      { code: "# Function to build Bell basis states", explanation: "Helper" },
      { code: "def make_bell_circuit(state_type):", explanation: "Circuit generator" },
      { code: "    qc = QuantumCircuit(2)", explanation: "2-qubit circuit" },
      { code: "    if 'Psi' in state_type:", explanation: "Flip target for Psi states" },
      { code: "        qc.x(1)", explanation: "X on wire 1" },
      { code: "    if '-' in state_type:", explanation: "Apply phase for minus states" },
      { code: "        qc.x(0)", explanation: "X on wire 0" },
      { code: "    qc.h(0)", explanation: "H on wire 0" },
      { code: "    qc.cx(0, 1)", explanation: "CNOT" },
      { code: "    return qc", explanation: "Return circuit" },
      { code: "", explanation: "" },
      { code: "bell_states = ['Phi+', 'Phi-', 'Psi+', 'Psi-']", explanation: "List of states" },
      { code: "print('=== The 4 Maximally Entangled Bell States in Qiskit ===')", explanation: "Header" },
      { code: "for name in bell_states:", explanation: "Loop states" },
      { code: "    circuit = make_bell_circuit(name)", explanation: "Build circuit" },
      { code: "    sv = Statevector.from_instruction(circuit)", explanation: "Compute statevector" },
      { code: "    probs = sv.probabilities_dict()", explanation: "Probabilities" },
      { code: "    prob_str = ', '.join([f'|{k}>: {v*100:.0f}%' for k, v in probs.items() if v > 0.01])", explanation: "Format string" },
      { code: "    print(f'{name:6s} -> Statevector: {sv.data.round(2)} | Non-zero: {prob_str}')", explanation: "Display" }
    ],
    executionFlow: [
      { number: 1, title: "Configure Input States", description: "Select appropriate X gate pre-rotations for each Bell state." },
      { number: 2, title: "Apply Hadamard and CNOT", description: "Transform product states into maximally entangled Bell pairs." },
      { number: 3, title: "Analyze Statevectors", description: "Display probability distribution demonstrating non-local correlations." }
    ],
    input: "Qiskit circuits for Phi+, Phi-, Psi+, and Psi-.",
    output: "Phi+ : [0.71, 0, 0, 0.71] -> |00>: 50%, |11>: 50%\nPhi- : [0.71, 0, 0, -0.71] -> |00>: 50%, |11>: 50%\nPsi+ : [0, 0.71, 0.71, 0] -> |01>: 50%, |10>: 50%\nPsi- : [0, 0.71, -0.71, 0] -> |01>: 50%, |10>: 50%",
    interpretation: "The four Bell states form an orthonormal basis for the 2-qubit Hilbert space. They are maximally entangled: no information resides in either individual qubit; all information is encoded in the shared quantum correlation.",
    colabInstructions: [
      "Click 'Copy for Google Colab' to copy the complete runnable script",
      "Open Google Colab at colab.research.google.com",
      "Paste into a code cell and press Shift + Enter to run",
...
- **Linear Systems**: Inverting $N \\times N$ sparse matrices in $O(\\text{poly}(\\log N))$ via the Harrow-Hassidim-Lloyd (HHL) algorithm.`,
  ...
  pythonHandsOn: {
    title: "Simulating a Molecular Hamiltonian with Qiskit Operators",
    description: "Model a molecular electronic Hamiltonian in Qiskit using Pauli operators and evaluate ground-state expectation values.",
    packages: ["qiskit", "numpy"],
    installCommand: "pip install qiskit numpy",
    imports: [
      { code: "from qiskit import QuantumCircuit", explanation: "Circuit builder" },
      { code: "from qiskit.quantum_info import SparsePauliOp, Statevector", explanation: "Pauli operator algebra" },
      { code: "import numpy as np", explanation: "NumPy math" }
    ],
    code: [
      { code: "# Define a simplified 2-qubit molecular Hamiltonian for hydrogen (H2)", explanation: "Hamiltonian setup" },
      { code: "# H = g0*II + g1*Z0 + g2*Z1 + g3*Z0Z1 + g4*X0X1", explanation: "Formula" },
      { code: "hamiltonian = SparsePauliOp.from_list([", explanation: "Create SparsePauliOp" },
      { code: "    ('II', -1.05),", explanation: "Identity energy offset" },
      { code: "    ('ZI',  0.39),", explanation: "Z on qubit 0" },
      { code: "    ('IZ', -0.39),", explanation: "Z on qubit 1" },
      { code: "    ('ZZ', -0.01),", explanation: "ZZ interaction" },
      { code: "    ('XX',  0.18),", explanation: "XX electron hopping term" },
      { code: "])", explanation: "Close list" },
      { code: "print('Molecular Hamiltonian Operator:')", explanation: "Label" },
      { code: "print(hamiltonian)", explanation: "Print terms" },
      { code: "", explanation: "" },
      { code: "# Prepare trial quantum state |01> (Hartree-Fock state for H2)", explanation: "Trial state" },
      { code: "qc = QuantumCircuit(2)", explanation: "2 qubits" },
      { code: "qc.x(1)  # Set qubit 1 to |1>, qubit 0 to |0>", explanation: "Prepare state |01>" },
      { code: "state = Statevector.from_instruction(qc)", explanation: "Extract statevector" },
      { code: "", explanation: "" },
      { code: "# Calculate Expectation Value: <psi | H | psi>", explanation: "Compute energy" },
      { code: "energy = state.expectation_value(hamiltonian).real", explanation: "Expectation value" },
      { code: "print(f'\\nTrial State: |01>')", explanation: "Print state" },
      { code: "print(f'Calculated Molecular Energy Expectation: {energy:.4f} Hartree')", explanation: "Energy in Hartree" },
      { code: "print('In VQE, parameterized rotation angles are tuned until this energy reaches the global minimum.')", explanation: "Summary" }
    ],
    executionFlow: [
      { number: 1, title: "Define Molecular Hamiltonian", description: "Represent electronic energy terms using Qiskit SparsePauliOp." },
      { number: 2, title: "Prepare Trial Wavefunction", description: "Construct quantum circuit representing molecular electron orbitals." },
      { number: 3, title: "Measure Energy Expectation", description: "Compute <psi | H | psi> to evaluate molecular ground-state binding energy." }
    ],
    input: "2-qubit SparsePauliOp Hamiltonian for H2 molecule.",
    output: "Molecular Hamiltonian: SparsePauliOp with II, ZI, IZ, ZZ, XX terms.\nCalculated Energy Expectation: ~ -1.8300 Hartree",
    interpretation: "Quantum chemistry is one of the highest-impact quantum computing applications. Simulating molecular orbitals on qubits allows calculating catalyst and drug binding energies without classical approximations.",
    colabInstructions: [
      "Click 'Copy for Google Colab' to copy the complete runnable script",
      "Open Google Colab at colab.research.google.com",
      "Paste into a code cell and press Shift + Enter to run",
