...
3. **Foundation for all advanced coursework**: Every sub-discipline—from computer vision to natural language processing—derives from foundational AI principles.`,
  ...
  pythonHandsOn: {
    title: 'Lesson 1: First Steps from Classical Prediction to a One-Qubit Quantum Circuit',
    description: 'Bridge classical linear regression prediction with quantum state generation using Qiskit Hadamard superposition and measurement on Aer simulator.',
    packages: ['scikit-learn', 'numpy', 'qiskit', 'qiskit-aer'],
    installCommand: 'pip install scikit-learn numpy qiskit qiskit-aer',
    imports: [
      { code: 'from sklearn.linear_model import LinearRegression', explanation: 'Classical ML linear regression model' },
      { code: 'import numpy as np', explanation: 'Numerical vector and matrix computations' },
      { code: 'from qiskit import QuantumCircuit', explanation: 'Assemble quantum circuits in Qiskit' },
      { code: 'from qiskit_aer import Aer', explanation: 'Qiskit Aer quantum simulator backend' },
    ],
    code: [
      { code: '# --- Part 1: Classical Machine Learning Prediction ---', explanation: 'Section header' },
      { code: 'X = np.array([1, 2, 3, 4, 5]).reshape(-1, 1)', explanation: 'Input feature data matrix (reshaped to 2D)' },
      { code: 'y = np.array([2, 4, 6, 8, 10])', explanation: 'Target labels following y = 2x relation' },
      { code: '', explanation: '' },
      { code: '# Train Classical Linear Regression Model', explanation: 'Fit model to training points' },
      { code: 'model = LinearRegression()', explanation: 'Instantiate estimator' },
      { code: 'model.fit(X, y)', explanation: 'Fit line via Ordinary Least Squares' },
      { code: 'prediction = model.predict([[8]])', explanation: 'Predict output for X=8' },
      { code: 'print(f"Classical ML Prediction for X=8: {prediction[0]:.1f}")', explanation: 'Display predicted value (16.0)' },
      { code: '', explanation: '' },
      { code: '# --- Part 2: Quantum Computing 1-Qubit Circuit ---', explanation: 'Section header' },
      { code: 'qc = QuantumCircuit(1, 1)', explanation: 'Allocate 1 quantum bit and 1 classical readout bit' },
      { code: 'qc.h(0)', explanation: 'Apply Hadamard gate to create 50/50 superposition' },
      { code: 'qc.measure(0, 0)', explanation: 'Projectively measure qubit 0 into classical bit 0' },
      { code: '', explanation: '' },
      { code: '# Simulate using Aer simulator', explanation: 'Configure simulator' },
      { code: 'simulator = Aer.get_backend(\'aer_simulator\')', explanation: 'Load Aer simulator' },
      { code: 'job = simulator.run(qc, shots=1000)', explanation: 'Execute 1,000 measurement shots' },
      { code: 'result = job.result()', explanation: 'Collect simulation results' },
      { code: 'counts = result.get_counts()', explanation: 'Extract measurement frequency distribution' },
      { code: 'print(f"Quantum Measurement Counts: {counts}")', explanation: 'Print outcome tally (approx 500 for 0 and 500 for 1)' },
    ],
    executionFlow: [
      { number: 1, title: 'Classical ML Training', description: 'Train a Scikit-Learn linear regression model on feature vector X and label vector y.' },
      { number: 2, title: 'Classical Prediction', description: 'Evaluate unseen input X=8 to observe deterministic output y=16.0.' },
      { number: 3, title: 'Quantum Circuit Construction', description: 'Construct a 1-qubit circuit and apply a Hadamard gate to put the qubit in superposition.' },
      { number: 4, title: 'Quantum Simulation', description: 'Execute 1,000 measurement shots on AerSimulator to demonstrate probabilistic collapse.' },
    ],
    input: 'Classical: X=8; Quantum: QuantumCircuit(1, 1) with H gate and measurement.',
    output: "Classical ML Prediction for X=8: 16.0\nQuantum Measurement Counts: {'0': 512, '1': 488}",
    interpretation: 'Contrasts classical deterministic prediction with quantum probabilistic superposition, introducing the foundational shift from binary bits to qubits.',
    colabInstructions: [
  ...
  learningResource: {
    title: 'Lesson 1: First Steps in Quantum Computing',
    pdfUrl: '/presentations/Lesson1.pdf',
    description: 'From Classical Prediction to a One-Qubit Quantum Circuit — Presentation by Dr. Syed Muzamil Basha.',
    lessonNumber: 1,
    pages: 19,
...
3. **Continuous adaptability**: As market trends or customer habits shift, ML models retrain and adjust dynamically.`,
  ...
  pythonHandsOn: {
    title: 'Lesson 2: Classical Machine Learning — House Price Prediction with Linear Regression',
    description: 'Train a linear regression model on house sizes and prices, predict prices for unseen square footage, and plot the regression line using matplotlib.',
    packages: ['scikit-learn', 'numpy', 'matplotlib'],
    installCommand: 'pip install scikit-learn numpy matplotlib',
    imports: [
      { code: 'import numpy as np', explanation: 'Array manipulation and matrix reshaping' },
      { code: 'import matplotlib.pyplot as plt', explanation: 'Plot data points and regression line' },
      { code: 'from sklearn.linear_model import LinearRegression', explanation: 'Ordinary Least Squares linear regression model' },
    ],
    code: [
      { code: '# Data: House sizes (sq ft) and corresponding prices', explanation: 'Define training dataset' },
      { code: 'X = np.array([500, 700, 900, 1100, 1300]).reshape(-1, 1)  # Feature (sq ft)', explanation: 'Reshape feature into 2D array' },
      { code: 'y = np.array([150000, 180000, 210000, 250000, 280000])  # Target (price in $)', explanation: 'Continuous target values' },
      { code: '', explanation: '' },
      { code: '# Create Linear Regression model', explanation: 'Instantiate linear regressor' },
      { code: 'model = LinearRegression()', explanation: 'Initialize estimator' },
      { code: '', explanation: '' },
      { code: '# Train the model on the data', explanation: 'Fit model parameters (slope and intercept)' },
      { code: 'model.fit(X, y)', explanation: 'Execute gradient/normal equation fitting' },
      { code: '', explanation: '' },
      { code: '# Predict price for a house with 1000 sq ft', explanation: 'Run inference on unseen feature' },
      { code: 'predicted_price = model.predict([[1000]])', explanation: 'Evaluate model at X=1000' },
      { code: 'print(f"Predicted Price for 1000 sq ft house: ${predicted_price[0]:,.2f}")', explanation: 'Print estimated price' },
      { code: '', explanation: '' },
      { code: '# Plot Data and Regression Line', explanation: 'Visualization with matplotlib' },
      { code: 'plt.scatter(X, y, color=\'blue\', label="Actual Prices")', explanation: 'Scatter plot of ground-truth points' },
      { code: 'plt.plot(X, model.predict(X), color=\'red\', label="Regression Line")', explanation: 'Fitted linear boundary' },
      { code: 'plt.xlabel("House Size (sq ft)")', explanation: 'Set X-axis label' },
      { code: 'plt.ylabel("House Price ($)")', explanation: 'Set Y-axis label' },
      { code: 'plt.title("Linear Regression - House Price Prediction (Lesson 2)")', explanation: 'Set plot title' },
      { code: 'plt.legend()', explanation: 'Display legend' },
      { code: 'plt.show()', explanation: 'Render plot figure' },
    ],
    executionFlow: [
      { number: 1, title: 'Dataset Formatting', description: 'Input feature square footage is converted to a 2D column vector and paired with price targets.' },
      { number: 2, title: 'Model Training', description: 'Scikit-Learn computes the optimal slope and intercept minimizing Mean Squared Error.' },
      { number: 3, title: 'Inference', description: 'Model predicts the price for a 1,000 sq ft house ($230,000.00).' },
      { number: 4, title: 'Visual Regression Line', description: 'Matplotlib plots the actual data points alongside the learned continuous regression line.' },
    ],
    input: 'House Sizes: [500, 700, 900, 1100, 1300] sq ft, Target Query: 1000 sq ft',
    output: 'Predicted Price for 1000 sq ft house: $230,000.00',
    interpretation: 'Demonstrates supervised learning regression: the model discovers the underlying linear relationship Price = m * Area + c and generalizes to unseen house sizes.',
    colabInstructions: [
      'Click "Copy for Google Colab" to copy the complete runnable script',
      'Open Google Colab at colab.research.google.com',
      'Paste into a code cell and press Shift + Enter to run',
  ...
  learningResource: {
    ...
    description: 'Data, prediction, and the bridge to modern AI — Presentation by Dr. Syed Muzamil Basha.',
    lessonNumber: 2,
    pages: 11,
    author: 'Dr. Syed Muzamil Basha',
  },
...
2. **Vibrant open-source ecosystem**: Hugging Face, LangChain, TensorFlow, PyTorch, and Scikit-Learn provide millions of pre-trained models accessible in one line of code.`,
  ...
  pythonHandsOn: {
    title: 'Lesson 4: Python Data Foundations — NumPy Arrays & Pandas DataFrames',
    description: 'Master core numerical and tabular data handling with NumPy arrays, matrix reshaping, random generation, and Pandas DataFrame manipulation.',
    packages: ['numpy', 'pandas'],
    installCommand: 'pip install numpy pandas',
    imports: [
      { code: 'import numpy as np', explanation: 'Core library for numerical arrays and vectorized linear algebra' },
      { code: 'import pandas as pd', explanation: 'High-performance DataFrame data structures for tabular data' },
    ],
    code: [
      { code: '# --- Part 1: NumPy Array Creation & Operations (Lesson 4.1) ---', explanation: 'Section header' },
      { code: 'arr = np.array([1, 2, 3, 4, 5])', explanation: 'Create a 1D NumPy array' },
      { code: 'print("NumPy Array:", arr)', explanation: 'Display array elements' },
      { code: 'print("Reshaped 5x1 Matrix:\n", arr.reshape(5, 1))', explanation: 'Reshape into 2D column matrix' },
      { code: 'print("Squared Array Elements:", arr ** 2)', explanation: 'Vectorized element-wise exponentiation' },
      { code: 'random_matrix = np.random.rand(3, 3)', explanation: 'Generate a 3x3 matrix of uniform random floats' },
      { code: 'print("Random 3x3 Matrix:\n", random_matrix.round(3))', explanation: 'Display random matrix' },
      { code: '', explanation: '' },
      { code: '# --- Part 2: Pandas DataFrames & Statistics (Lesson 4.2) ---', explanation: 'Section header' },
      { code: 'data = {', explanation: 'Construct sample dictionary' },
      { code: '    "Name": ["Alice", "Bob", "Charlie"],', explanation: 'Categorical string column' },
      { code: '    "Age": [25, 30, 35],', explanation: 'Numerical integer column' },
      { code: '    "Salary": [50000, 60000, 70000]', explanation: 'Numerical salary column' },
      { code: '}', explanation: 'Close dictionary' },
      { code: 'df = pd.DataFrame(data)', explanation: 'Create structured Pandas DataFrame' },
      { code: 'print("\nPandas DataFrame:\n", df)', explanation: 'Display tabular DataFrame' },
      { code: 'print("\nDescriptive Statistics:\n", df.describe())', explanation: 'Compute summary metrics (mean, std, min, max)' },
      { code: 'filtered_df = df[df["Age"] > 30]', explanation: 'Filter rows based on condition Age > 30' },
      { code: 'print("\nEmployees with Age > 30:\n", filtered_df)', explanation: 'Display filtered subset' },
    ],
    executionFlow: [
      { number: 1, title: 'NumPy Array Construction', description: 'Instantiate and reshape contiguous numerical vectors.' },
      { number: 2, title: 'Vectorized Mathematics', description: 'Perform instant C-level element-wise squaring without slow Python for-loops.' },
      { number: 3, title: 'DataFrame Ingestion', description: 'Load structured records into an indexed Pandas DataFrame.' },
      { number: 4, title: 'Statistical Aggregation & Filtering', description: 'Compute summary statistics and execute conditional row filtering.' },
    ],
    input: 'NumPy array [1,2,3,4,5] and Employee dictionary {Name, Age, Salary}',
    output: 'Array: [1 2 3 4 5]\nSquared: [ 1  4  9 16 25]\nMean Age: 30.0, Mean Salary: $60,000\nEmployees > 30: Charlie (Age 35, Salary $70,000)',
    interpretation: 'NumPy provides the high-speed numerical array backend, while Pandas enables clean tabular data wrangling — forming the indispensable data foundation for all AI and quantum preprocessing pipelines.',
    colabInstructions: [
      'Click "Copy for Google Colab" to copy the complete runnable script',
      'Open Google Colab at colab.research.google.com',
      'Paste into a code cell and press Shift + Enter to run',
  ...
  learningResource: {
    ...
    description: 'NumPy, pandas, preprocessing, and classification pipelines — Presentation by Dr. Syed Muzamil Basha.',
    lessonNumber: 4,
    pages: 17,
    author: 'Dr. Syed Muzamil Basha',
  },
