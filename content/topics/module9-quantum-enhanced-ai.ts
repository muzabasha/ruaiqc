...
export const module9Topics: Topic[] = [
  ...
  },
  {
    ...
    pythonHandsOn: {
      title: 'Solving a Binary Optimization Problem with QAOA Simulation',
      description: 'Define a simple 2-qubit Max-Cut or QUBO optimization problem and optimize variational angles using SciPy.',
      packages: ['numpy', 'scipy'],
      installCommand: 'pip install numpy scipy',
      imports: [
        { code: 'import numpy as np', explanation: 'Import NumPy for matrix math' },
        { code: 'from scipy.optimize import minimize', explanation: 'Import classical optimizer for outer loop' },
      ],
      code: [
        { code: '# Cost function for Max-Cut on 2 nodes connected by edge: minimize -(1 - Z0*Z1)/2', explanation: 'Cost Hamiltonian matrix' },
        { code: 'Z = np.array([[1, 0], [0, -1]])', explanation: 'Pauli Z matrix' },
        { code: 'I = np.eye(2)', explanation: '2x2 identity matrix' },
        { code: 'ZZ = np.kron(Z, Z)', explanation: 'Two-qubit Pauli Z cross Z' },
        { code: 'H_C = 0.5 * (np.eye(4) - ZZ)', explanation: 'Cost Hamiltonian (eigenvalues: 0 for |00>,|11>; 1 for |01>,|10>)' },
        { code: 'X = np.array([[0, 1], [1, 0]])', explanation: 'Pauli X matrix' },
        { code: 'H_M = np.kron(X, I) + np.kron(I, X)', explanation: 'Mixer Hamiltonian' },
        { code: 'def qaoa_energy(params):', explanation: 'Compute expectation value for gamma and beta' },
        { code: '    gamma, beta = params', explanation: 'Unpack angles' },
        { code: '    psi0 = np.ones(4) / 2.0', explanation: 'Uniform superposition state |+>|+>' },
        { code: '    from scipy.linalg import expm', explanation: 'Matrix exponential helper' },
        { code: '    U_C = expm(-1j * gamma * H_C)', explanation: 'Cost unitary' },
        { code: '    U_M = expm(-1j * beta * H_M)', explanation: 'Mixer unitary' },
        { code: '    state = U_M @ (U_C @ psi0)', explanation: 'Apply single QAOA layer' },
        { code: '    expectation = np.vdot(state, H_C @ state).real', explanation: 'Expectation value <H_C>' },
        { code: '    return -expectation  # Maximize cut', explanation: 'Return negative for minimization' },
        { code: 'res = minimize(qaoa_energy, [0.5, 0.5], method="COBYLA")', explanation: 'Run classical optimizer' },
        { code: 'print("Optimal (gamma, beta):", res.x)', explanation: 'Print parameters' },
        { code: 'print("Max expected cut value:", -res.fun)', explanation: 'Print optimized energy' },
      ],
      executionFlow: [
        { number: 1, title: 'Formulate Hamiltonian', description: 'Define Pauli operators representing graph cut objective.' },
        { number: 2, title: 'Simulate State Evolution', description: 'Apply alternating cost and mixer unitaries on uniform superposition.' },
        { number: 3, title: 'Optimize Parameters', description: 'Use classical COBYLA optimizer to tune angles until ground state probability is maximized.' },
      ],
      input: 'Initial parameters: gamma=0.5, beta=0.5',
      output: 'Optimal angles found; Expected cut value ~ 1.0 (indicating optimal cut |01> or |10>)',
      interpretation: 'The quantum circuit concentrates probability amplitude onto the states |01> and |10> which correctly cut the edge between node 0 and node 1.',
      colabInstructions: [
        'Open Google Colab',
        'Paste the code and run with Shift+Enter',
        'Observe how the expected cut approaches 1.0',
      ],
    },
    mcqs: [
      {
