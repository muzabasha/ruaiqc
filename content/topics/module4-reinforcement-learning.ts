...
To find the best move, look up your current row and pick the column with the highest number!`,
  ...
  learningResource: {
    ...
    description: 'Q-learning, Bellman optimality equation, Q-table updates, and exploration-exploitation balance.',
    lessonNumber: 6,
    pages: 12,
    author: 'Dr. Syed Muzamil Basha',
  },
  pythonHandsOn: {
    title: 'Q-Table Optimization on Gymnasium FrozenLake-v1',
    description: 'Train an agent to navigate the FrozenLake environment using a 16x4 Q-table, epsilon-greedy exploration, and the Bellman temporal difference update equation.',
    packages: ['gymnasium', 'numpy'],
    installCommand: 'pip install gymnasium numpy',
    imports: [
      { code: 'import numpy as np', explanation: 'NumPy for 2D Q-matrix math' },
      { code: 'import gymnasium as gym', explanation: 'Gymnasium reinforcement learning environment suite' },
    ],
    code: [
      { code: '# Create FrozenLake environment (4x4 grid = 16 states, 4 actions)', explanation: 'Environment creation' },
      { code: 'env = gym.make("FrozenLake-v1", is_slippery=False)', explanation: 'Deterministic 4x4 grid' },
      { code: 'state_size = env.observation_space.n', explanation: '16 discrete states' },
      { code: 'action_size = env.action_space.n', explanation: '4 discrete actions: Left, Down, Right, Up' },
      { code: '', explanation: '' },
      { code: '# Initialize Q-table with zeros', explanation: 'Initialize Q-table' },
      { code: 'Q_table = np.zeros((state_size, action_size))', explanation: '16x4 matrix' },
      { code: '', explanation: '' },
      { code: '# Hyperparameters', explanation: 'Parameters' },
      { code: 'alpha = 0.1        # Learning rate', explanation: 'Alpha step size' },
      { code: 'gamma = 0.99       # Discount factor', explanation: 'Gamma future discount' },
      { code: 'epsilon = 1.0      # Initial exploration rate', explanation: 'Exploration probability' },
      { code: 'epsilon_decay = 0.995', explanation: 'Decay rate per episode' },
      { code: 'epsilon_min = 0.01', explanation: 'Minimum exploration floor' },
      { code: 'episodes = 2000    # Number of games', explanation: 'Training duration' },
      { code: '', explanation: '' },
      { code: '# Training Loop', explanation: 'Training loop' },
      { code: 'for episode in range(episodes):', explanation: 'Loop over episodes' },
      { code: '    state = env.reset()[0]', explanation: 'Reset environment to start state' },
      { code: '    done = False', explanation: 'Episode flag' },
      { code: '', explanation: '' },
      { code: '    while not done:', explanation: 'Step through episode' },
      { code: '        # Epsilon-greedy action selection', explanation: 'Action choice' },
      { code: '        if np.random.rand() < epsilon:', explanation: 'Explore' },
      { code: '            action = np.random.choice(action_size)', explanation: 'Sample random action' },
      { code: '        else:', explanation: 'Exploit' },
      { code: '            action = np.argmax(Q_table[state])', explanation: 'Greedy action from Q-table' },
      { code: '', explanation: '' },
      { code: '        # Take action and observe transition', explanation: 'Environment step' },
      { code: '        next_state, reward, done, _, _ = env.step(action)', explanation: 'Step transition' },
      { code: '', explanation: '' },
      { code: '        # Update Q-table using the Bellman equation', explanation: 'Q-value update' },
      { code: '        Q_table[state, action] = Q_table[state, action] + alpha * (', explanation: 'Temporal difference update' },
      { code: '            reward + gamma * np.max(Q_table[next_state]) - Q_table[state, action]', explanation: 'TD error calculation' },
      { code: '        )', explanation: 'Close update' },
      { code: '', explanation: '' },
      { code: '        state = next_state', explanation: 'Transition to next state' },
      { code: '', explanation: '' },
      { code: '    # Decay epsilon to shift from exploration to exploitation', explanation: 'Epsilon decay' },
      { code: '    epsilon = max(epsilon * epsilon_decay, epsilon_min)', explanation: 'Reduce epsilon' },
      { code: '', explanation: '' },
      { code: 'print("Training complete! Non-zero learned Q-values:\\n", np.round(Q_table[Q_table > 0][:5], 4))', explanation: 'Display learned values' },
    ],
    executionFlow: [
      { number: 1, title: 'Environment Setup', description: 'Creates a 16-state FrozenLake grid and initializes a 16x4 Q-table with zeros.' },
      { number: 2, title: 'Epsilon-Greedy Interaction', description: 'Agent balances exploratory random actions with exploitative argmax actions, decaying epsilon from 1.0 to 0.01.' },
      { number: 3, title: 'Bellman Propagation', description: 'Whenever the agent reaches the goal (reward=1), the reward propagates backwards across the Q-table, revealing the optimal trajectory.' },
    ],
    input: '16x4 FrozenLake environment trained over 2,000 episodes',
    output: 'Training complete! Non-zero learned Q-values:\n[0.5832 0.6591 0.7451 0.8428 0.9510]',
    interpretation: 'The Q-table converges to the optimal navigation policy, ensuring the agent takes the safest, shortest route across the frozen lake to reach the goal.',
    colabInstructions: ['Run in Google Colab to watch the agent learn optimal paths.'],
  },
  mcqs: [
    {
      id: 'q1',
...
2. **Fixed Target Network $\\boldsymbol{\\theta}^-$**: Decoupling target generation $y_t = r + \\gamma \\max_{a'} Q(s', a'; \\boldsymbol{\\theta}^-)$ with periodic parameter synchronization $\\boldsymbol{\\theta}^- \\leftarrow \\boldsymbol{\\theta}$.`,
  ...
  learningResource: {
    title: 'Lesson 6: Reinforcement Learning Foundations',
    pdfUrl: '/presentations/Lesson6.pdf',
    description: 'Deep Q-Networks (DQN), replay buffer, neural network value function approximation, and CartPole balancing.',
    lessonNumber: 6,
    pages: 12,
    author: 'Dr. Syed Muzamil Basha',
  },
  pythonHandsOn: {
    title: 'Deep Q-Network (DQN) with Experience Replay on CartPole-v1',
    description: 'Implement a full Deep Q-Network (DQN) in PyTorch to solve Gymnasium CartPole-v1 using an experience replay buffer (deque) and epsilon-greedy exploration.',
    packages: ['torch', 'gymnasium', 'numpy'],
    installCommand: 'pip install torch gymnasium numpy',
    imports: [
      ...
      { code: 'import torch.optim as optim', explanation: 'Adam optimizer' },
      { code: 'import gymnasium as gym', explanation: 'Gymnasium CartPole simulation' },
      { code: 'from collections import deque', explanation: 'Double-ended queue for experience replay buffer' },
      { code: 'import random', explanation: 'Random mini-batch sampling' },
    ],
    code: [
      { code: '# Setup Gymnasium CartPole Environment', explanation: 'Environment' },
      { code: 'env = gym.make("CartPole-v1")', explanation: 'CartPole physics simulation' },
      { code: 'state_size = env.observation_space.shape[0]  # 4 features: [pos, vel, angle, angular_vel]', explanation: 'Input dimensions' },
      { code: 'action_size = env.action_space.n               # 2 actions: [push left, push right]', explanation: 'Output action count' },
      { code: '', explanation: '' },
      { code: '# Define Deep Q-Network Architecture (MLP)', explanation: 'Network' },
      { code: 'class DQN(nn.Module):', explanation: 'Subclass nn.Module' },
      { code: '    def __init__(self, state_size, action_size):', explanation: 'Constructor' },
      { code: '        super(DQN, self).__init__()', explanation: 'Initialize base' },
      { code: '        self.fc1 = nn.Linear(state_size, 24)', explanation: 'Input layer (4 -> 24)' },
      { code: '        self.fc2 = nn.Linear(24, 24)', explanation: 'Hidden layer (24 -> 24)' },
      { code: '        self.fc3 = nn.Linear(24, action_size)', explanation: 'Output Q-values (24 -> 2)' },
      { code: '    def forward(self, x):', explanation: 'Forward computation' },
      { code: '        x = torch.relu(self.fc1(x))', explanation: 'Layer 1 + ReLU' },
      { code: '        x = torch.relu(self.fc2(x))', explanation: 'Layer 2 + ReLU' },
      { code: '        return self.fc3(x)', explanation: 'Linear output Q-values' },
      { code: '', explanation: '' },
      { code: 'model = DQN(state_size, action_size)', explanation: 'Instantiate DQN model' },
      { code: 'optimizer = optim.Adam(model.parameters(), lr=0.001)', explanation: 'Adam optimizer' },
      { code: 'loss_function = nn.MSELoss()', explanation: 'Mean squared error loss' },
      { code: '', explanation: '' },
      { code: '# Replay buffer and hyperparameters', explanation: 'Buffer setup' },
      { code: 'memory = deque(maxlen=2000)  # Experience replay buffer', explanation: 'Circular memory' },
      { code: 'gamma = 0.95                 # Discount factor', explanation: 'Future reward discount' },
      { code: 'epsilon = 1.0                # Initial exploration', explanation: 'Exploration rate' },
      { code: 'epsilon_min = 0.01', explanation: 'Exploration floor' },
      { code: 'epsilon_decay = 0.995', explanation: 'Decay per episode' },
      { code: 'batch_size = 64', explanation: 'Mini-batch training size' },
      { code: '', explanation: '' },
      { code: 'def replay():', explanation: 'Mini-batch replay training function' },
      { code: '    if len(memory) < batch_size:', explanation: 'Wait for buffer warm-up' },
      { code: '        return', explanation: 'Return early' },
      { code: '    minibatch = random.sample(memory, batch_size)', explanation: 'Uniform random sampling' },
      { code: '    for state, action, reward, next_state, done in minibatch:', explanation: 'Iterate transitions' },
      { code: '        target = reward', explanation: 'Base target' },
      { code: '        if not done:', explanation: 'Bootstrap future Q' },
      { code: '            target += gamma * torch.max(model(torch.FloatTensor(next_state))).item()', explanation: 'Bellman target' },
      { code: '        output = model(torch.FloatTensor(state))[action]', explanation: 'Predicted Q(s, a)' },
      { code: '        loss = loss_function(output.view(-1), torch.tensor([target], dtype=torch.float32))', explanation: 'MSE loss' },
      { code: '        optimizer.zero_grad()', explanation: 'Zero gradients' },
      { code: '        loss.backward()', explanation: 'Backpropagation' },
      { code: '        optimizer.step()', explanation: 'Weight update' },
      { code: '', explanation: '' },
      { code: '# Train for sample demonstration episodes', explanation: 'Training loop' },
      { code: 'for episode in range(10):', explanation: 'Episode loop' },
      { code: '    state = env.reset()[0]', explanation: 'Reset episode' },
      { code: '    done = False', explanation: 'Done flag' },
      { code: '    total_reward = 0', explanation: 'Reward tally' },
      { code: '    while not done:', explanation: 'Step loop' },
      { code: '        if np.random.rand() < epsilon:', explanation: 'Explore' },
      { code: '            action = random.randrange(action_size)', explanation: 'Random push' },
      { code: '        else:', explanation: 'Exploit' },
      { code: '            action = torch.argmax(model(torch.FloatTensor(state))).item()', explanation: 'Argmax action' },
      { code: '        next_state, reward, done, _, _ = env.step(action)', explanation: 'Step' },
      { code: '        memory.append((state, action, reward, next_state, done))', explanation: 'Store memory' },
      { code: '        state = next_state', explanation: 'Next state' },
      { code: '        total_reward += reward', explanation: 'Accumulate reward' },
      { code: '    replay()', explanation: 'Train network on replay buffer' },
      { code: '    epsilon = max(epsilon * epsilon_decay, epsilon_min)', explanation: 'Decay epsilon' },
      { code: '    print(f"Episode {episode+1}, Total Reward: {total_reward:.0f}")', explanation: 'Log episode' },
    ],
    executionFlow: [
      { number: 1, title: 'Neural Approximation', description: 'A PyTorch MLP replaces the discrete Q-table, mapping 4 continuous state features to 2 action Q-values.' },
      { number: 2, title: 'Experience Replay', description: 'Transitions (s, a, r, s\', done) are stored in a 2000-step deque and uniformly sampled in batches of 64 to break temporal correlation.' },
      { number: 3, title: 'Bellman Target Minimization', description: 'The network weights are updated via Adam optimizer to minimize the MSE discrepancy between current predictions and bootstrapped Bellman targets.' },
    ],
    input: 'Continuous 4D CartPole state space [cart position, cart velocity, pole angle, pole angular velocity]',
    output: 'Episode 1, Total Reward: 18\nEpisode 2, Total Reward: 24\nEpisode 3, Total Reward: 31\n...',
    interpretation: 'The deep neural network successfully learns to balance the CartPole upright by continuously updating its internal weights from experience replay memories.',
    colabInstructions: ['Run in Google Colab to observe the agent mastering pole balancing.'],
  },
  mcqs: [
    {
      id: 'q1',
